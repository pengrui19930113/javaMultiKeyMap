...
//    protected 如果用此修饰符修饰 则包外的子类可以获取到改对象的引用
    ...
    @Override
    public void onDestroy(Object... params) {
        post(this::_destroy);
        if(params.length>0&&Objects.equals(params[0],0)){
            new Thread(()->{
                while(!isFinished()){
                    onTimer();
                    try {
    ...
    @Override
    public GamingInitData gamingData() {
        final GamingInitData gamingData = new GamingInitData() {
            @Override
