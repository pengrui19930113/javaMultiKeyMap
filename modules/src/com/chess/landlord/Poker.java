...
public class Poker implements Comparable<Poker> {
   ...
   */
    ...
    protected Poker(byte type, byte value){
        this(type,value,false);
    }
    protected Poker(byte type, byte value, boolean start){
    ...
    public boolean isHeart(){
        return (code &TYPE_MASK) == HEART;
    }
    public boolean isDiamond(){
        return (code &TYPE_MASK) ==(BYTE_MASK&DIAMOND);
    }
    public boolean isClub(){
        return (code &TYPE_MASK) == (BYTE_MASK&CLUB);
    }
    @Override
    ...
    public static final Poker[] NORMAL = {
            ...
            ,new Poker(STAR_TYPE,ST)
    };
    public enum Type{
        SPADE(Poker.SPADE),HEART(Poker.HEART),CLUB(Poker.CLUB),DIAMOND(Poker.DIAMOND)
        ,JOKER(JOKER_TYPE),STAR(STAR_TYPE)
        ;
    ...
    public enum Value{
        _3(Poker._3),_4(Poker._4),_5(Poker._5)
        ,_6(Poker._6),_7(Poker._7),_8(Poker._8)
        ,_9(Poker._9),_10(Poker._10)
        ,J(Poker.J),Q(Poker.Q),K(Poker.K)
        ,A(Poker.A),_2(__2),STAR(ST)
        ,BLANK(BJ),RED(RJ)
        ;
        public final int code;
        Value(int code) {
    ...
    public static Poker create(Type t,Value v,boolean start){
        switch (t){
            case SPADE ,HEART,CLUB,DIAMOND ->{
                switch (v){
                    case STAR,RED,BLANK -> throw new IllegalArgumentException();
                    default -> {return new Poker((byte)t.code,(byte)v.code,start);}
                }
            }
            case JOKER -> {
                switch (v){
                    case RED ,BLANK-> {return new Poker((byte)t.code,(byte)v.code);}
                    default -> throw new IllegalArgumentException();
                }
            }
            case STAR -> {
                switch (v){
                    case STAR -> {return new Poker((byte)t.code,(byte)v.code,true);}
                    default -> throw new IllegalArgumentException();
                }
            }
        }
    ...
    public static Poker create(Type t,Value v){
        return create(t,v,false);
    }

    public static void main(String[] args) {
        final Poker poker = create(Type.STAR, Value.STAR,false);
